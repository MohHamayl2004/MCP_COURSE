...
- `get_spending_summary` (P0) — Total spending + per-category breakdown for a month. Input: month. Output: `{ total, byCategory: { category: amount } }`.
...
- [ ] `get_spending_summary` returns the correct total and category breakdown for fixture data.
...
Planned read-only resources, not implemented yet:

- `expenses://fixture` — the raw CSV, so the model can read the data without a tool call
- `expenses://categories` — the list of categories currently in use
- `docs://data-plan` — the data plan, for context on where data comes from

Week 3 scope is tools only.